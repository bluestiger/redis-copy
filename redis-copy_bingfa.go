...
var (
	...
	targetAddr = kingpin.Flag("dstaddr", "Use -h <目标IP地址>").Default("").Short('h').String()
	targetPort = kingpin.Flag("dstport", "Use -p <目标端口>").Default("6379").Short('p').String()
	targetAuth = kingpin.Flag("dstauth", "Use -a <目标认证密码>").Default("").Short('a').String()
	targetDbs  = kingpin.Flag("dstdbs", "Use -d <目标数据库>").Default("0").Short('d').Int()

	sourcePool, targetPool *redis.Pool
	contents               string
	multipleCopy           = kingpin.Flag("mcopy", "Use -m <要复制键值如：test*>").Default("").Short('m').String()
	outputlog              = kingpin.Flag("output", "Use -out <导出文件名>").Default("./output.log").Short('o').String()
)
